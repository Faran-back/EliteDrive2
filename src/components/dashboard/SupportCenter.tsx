...
const CommentForm: React.FC<{ incidentId: string; onSend: (id: string, message: string) => Promise<void> }> = ({ incidentId, onSend }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2 items-center mt-2.5">
      <input
        type="text"
        placeholder="Type a remark or reply..."
        value={msg}
        onChange={(e) => setMsg(e.target.value)}
        disabled={isSending}
        className="flex-1 bg-slate-50 text-xs text-slate-800 font-semibold px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 disabled:opacity-60 placeholder-slate-400"
      />
      <button
        type="submit"
        disabled={isSending || !msg.trim()}
        className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-sm transition-all flex items-center gap-1.5 whitespace-nowrap"
      >
        <Send size={11} />
                      ...
                      <div key={inc.id} className="bg-white p-6 rounded-3xl border border-slate-150 shadow-sm space-y-4 hover:shadow-md transition-all">
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                          <div className="flex items-center gap-1.5 text-slate-800">
                            ...
                            <h5 className="text-[11px] font-extrabold uppercase tracking-wider text-slate-900">Remarks & Cross-Discussion</h5>
                          </div>
                          
                          <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                            {(!inc.comments || inc.comments.length === 0) ? (
                              <p className="text-[10px] text-slate-400 italic">No previous discussion comments logged. Use the form below to post remarks or coordinate with the customer.</p>
                            ) : (
                              inc.comments.map((comment: any) => {
                                const isStaff = comment.senderRole === 'admin' || comment.senderRole === 'manager';
                                return (
                                  <div key={comment.id} className={`p-2.5 rounded-2xl border text-[11px] leading-relaxed max-w-[85%] ${
                                    isStaff 
                                      ? 'bg-blue-50/60 border-blue-100 text-blue-900 ml-auto mr-0' 
                                      : 'bg-slate-50/80 border-slate-200 text-slate-800 ml-0 mr-auto'
                                  }`}>
                                    <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                                      <span className="font-extrabold">{comment.senderName}</span>
                                      <span className={`px-1 py-0.1 rounded text-[8px] font-bold uppercase tracking-widest ${
                                        isStaff ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-700'
                                      }`}>
                                        {comment.senderRole}
                                      </span>
                                      <span className="text-[9px] text-slate-400 ml-auto">{new Date(comment.createdAt).toLocaleString()}</span>
                                    </div>
